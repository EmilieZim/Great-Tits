...
#EZ: Why is degree gone from the model actually? It had no interactions with the other predictors but we could maybe still leave it in the final model (without pairing it with other predictiors in the form of an interaction)

#EZ: I want to see the difference between great tits and nuthaches. I decide that nuthaches can be the intercept
str(network.pos.all.seasons$species)
network.pos.all.seasons$species<- as.factor(network.pos.all.seasons$species)
levels(network.pos.all.seasons$species)
network.pos.all.seasons$species <- factor(network.pos.all.seasons$species, levels = c("NUTHA", "GRETI","BLUTI",  "MARTI"))
summary(red_model)
#it did not work, probably because I would have to run the red_model with these new changes (I can't since my computer is not cooperative :/)

##And same for season
str(network.pos.all.seasons$season)
network.pos.all.seasons$season<- as.factor(network.pos.all.seasons$season)
levels(network.pos.all.seasons$season)
summary(red_model)

#EZ: In response to the previous comment, I will try to compare the different levels with emmeans

library(emmeans)
em_sp <- emmeans(red_model,~ species, type = "response")
contrast1 <- contrast(em_sp, method = "pairwise")
contrast1



em_seas <- emmeans(red_model,~ season, type = "response")
contrast2 <- contrast(em_seas, method = "pairwise")
contrast2
...
em_species_logit <- regrid(em_species,
                           transform = "logit")
contrast <- contrast(em_species_logit, method = "pairwise")





em <- emmeans(red_model, "species", epred=TRUE)

...
dim(network.pos.all.seasons)
# 14495 rows

# SW: we need to remove all 'groups' with only one bird
network.pos.all.seasons <- network.pos.all.seasons[network.pos.all.seasons$group.size>1,]

dim(network.pos.all.seasons)
# 13394 rows

# make a binary varaible out of the order with 'leader' versus 'follower'
...
# we can use the funciton icc (intra class correlation coefficient) from package performance
...
# SW: I have removed the rest of the code below to keep it clean. 

#####In the literature, what has been done so far and is similar to this study
